...
2. tenancy(Administrators)権限にてOCI Console login後
OCI Cloud Shellを開く

3. Cloud Shellのメニューからoci-cloud-guard-check.shをupload

4. oci-cloud-guard-check.sh を実行
"tenancy名_実行日時"の名前のDirectoryが新規に作成され以下にCloud Guardの各種Configuration関連fileが作成される

5.上記Directory自体のzipも作成されるのでCloud Shellのメニューから自PCにDownload

6.自PCで上記zipを解凍
Cloud Shell上でシェル実行した結果の"tenancy名_実行日時"の名前のDirectoryが展開される

7. すでにDownloadしてあるgenerate_rof_10.py、generate_problem_details_doc_5.py
を上記Directory下にコピー
...
5. "RecipeConfig_実行日時"の名前のDirectoryが新規に作成され配下にCloud Guardの各種Recipe構成file(Detectorタイプ単位:Config)が作成される。必要に応じてCloud Shellのメニューから自PCにダウンロード
